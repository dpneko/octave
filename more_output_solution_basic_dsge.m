...
% Thomas Hintermaier, hinterma@uni-bonn.de, January 7, 2020
...
plot(k_t,'g-x');
set(gca,"fontsize", 18)
title('productivity state, consumption, capital');
...
% plot state of technology shock and shock innovations
figure(555);
 sample_period = 1:T;
 plot(sample_period,s_t(sample_period),...
      sample_period,v_t(sample_period),...
      sample_period,zeros(size(s_t)));
 set(gca,"fontsize", 18)
 legend('s_t','v_t')


y_t = s_t + alpha*k_t(1:T); 


timeline = 1:T;
figure(111);
plot(timeline,c_t,timeline,y_t);
set(gca,"fontsize", 18)
legend('consumption','GDP')


corr_c_y = corr(c_t,y_t) % correlation

std_c = std(c_t) % standard deviation
std_y = std(y_t)



figure(21);
hist(c_t,20);   % histogram

figure(22);
scatter(c_t,y_t); % scatter plot
set(gca,"fontsize", 18)
xlabel('consumption');
ylabel('GDP');
...
expect_error_c_tp1 = NaN*zeros(T-1,1);
for t = 1:T-1;
   expect_error_c_tp1(t) = c_coeff_k*k_t(t+1) + c_coeff_s*rho*s_t(t) - c_t(t+1);
end;



figure(30);
plot(expect_error_c_tp1);
set(gca,"fontsize", 18)
title('expectational error in consumption')

%
%mean_expect_error_c = mean(expect_error_c_tp1)
%
%
%
%figure(32);
%scatter(v_t(2:end),expect_error_c_tp1)
%set(gca,"fontsize", 18)
%xlabel('fundamental shock innovation')
%ylabel('expectational error of consumption')
%
%
%
%
%figure(33);
%plot(v_t(2:end),expect_error_c_tp1,'o',v_t(2:end),-c_coeff_s*v_t(2:end),'r')
%%%
%%%
%%%
